...
const SearchBox = () => {
  ...
  const filterRef = useRef(null); // New ref for filter dropdowns
      ...
      if (search || propertyType || status || bhkType) {
        ...
        const results = propertyData.filter((item) => {
          const matchesSearch = search
            ? ["title", "description", "address", "city", "state", "constructionYear"].some((key) =>
          ...
          const matchesBhk = bhkType
            ? item.floorPlan.some(
                (plan) =>
                  plan.type.toLowerCase().replaceAll(" ", "") === bhkType.toLowerCase().replaceAll(" ", "")
              )
            : true;
  ...
  const handleSearchSubmit = async (e) => {
    ...
    if (selectedIndex >= 0 && filteredData[selectedIndex]) {
      ...
      if (customerAuth?.token) {
        try {
          await axios.post(`${baseUrl}/save-search-history`, { query }, {
      ...
      {isHideSearch && filteredData.length > 0 && (
        ...
        >
          {filteredData.map((item, index) => (
            <div
              key={item._id}
              onClick={() => navigate(`/projects/${item.title.replaceAll(" ", "-")}/${item._id}`)}
