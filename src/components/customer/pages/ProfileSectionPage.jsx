...
const ProfileSectionPage = () => {
  ...
  useEffect(() => {
    ...
    if (!customerAuth || !customerAuth.token) {
      toast.error("Please log in to access your profile", { position: "top-left" });
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      const customerAuth = JSON.parse(localStorage.getItem("customerAuth"));
      if (!customerAuth || !customerAuth.token) {
        toast.error("Please log in to update profile", { position: "top-left" });
        navigate("/customer-sign-in");
        return;
      }
      ...
      const res = await axios.put(`${baseUrl}/customer-update-profile`, formData, config);
